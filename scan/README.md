...
| [nmap-shard.yml](nmap-shard.yml) | Nmap (shard) | v2 child launched with `--count`: each instance scans only its 1-based slice ($SATORI_COUNT/$SATORI_COUNT_INDEX) of the host list and writes nmap.txt. PORTS is an nmap `-p` spec (e.g. `1-1000`, `80,443`, `-` for all 65535) |
